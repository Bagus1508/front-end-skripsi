...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (editMode.value) {
      url = `${baseURLApi}/non-academic-reports/update/${getData.id}`;
      method = 'put';
    } else {
      url = `${baseURLApi}/non-academic-reports/store`;
      method = 'post';
    }
    ...
    <div class="card flex justify-center">
        <Dialog v-model:visible="showCreateModal" modal v-model:header="titleModal" :style="{ width: '50vw' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }" style="background-color: white;border-radius: 5px;">
            <form @submit.prevent="handleSubmit">
                ...
                <InputText label="Pencapaian" name="achievement" v-model="getData.achievement"/>
