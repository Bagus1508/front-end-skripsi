...
onMounted(async () => {
  try {
    ...
    const [answersResponse, questionsResponse] = await Promise.all([
        fetch(`${baseURLApi}/answers/${route.query.question_id}`),
        fetch(`${baseURLApi}/questions/${route.query.question_id}`)
    ]);
    ...
    answersList.value = answersResult.data;
    questionData.value = questionsResult.data;
...
const createModal = () => {
    ...
    titleModal.value = 'Tambah Jawaban';
    modalType.value = 'create';
    editMode.value = false;
    questionId.value = route.query.question_id;
};
...
const showDelete = async (data) => {
  try {
    ...
    if (confirmResult.isConfirmed) {
      ...
      await axios.delete(`${baseURLApi}/answers/delete/${id}`);

      // Hapus data dari state frontend
      answersList.value = answersList.value.filter(answersList => answersList.id !== id);
...
<template>
    ...
    <div class="mt-2">
        <div class="relative overflow-x-auto">
            ...
            <DataTable v-model:filters="filters" :value="answersList" resizableColumns columnResizeMode="fit" showGridlines paginator stripedRows :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]" tableStyle="margin-bottom: 10px;"
                ...
                <Column field="option" sortable header="Opsi"></Column>
                <Column field="description" sortable header="Deskripsi"></Column>
                <Column field="is_true_answer" sortable header="Is True"></Column>
                <Column field="attachment" header="Lampiran">
                    <template #body="slotProps">
                        <img :src="slotProps.data.attachment" alt="Lampiran" width="100" height="100" style="object-fit: cover;">
                    </template>
                </Column>
            </DataTable>
        </div>
